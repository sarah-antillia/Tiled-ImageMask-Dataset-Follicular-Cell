...
class TiledImageMaskDatasetGenerator:
  ...
  def create_mask_files(self, mask_file, index):
    ...
    if self.augmentation:
        self.augment(resized, basename, self.output_masks_dir, border=(0, 0, 0), ismask=True)
  ...
  def create_image_files(self, image_file, index):
    ...
    print("--- Saved {}".format(filepath))
    if self.augmentation:
        self.augment(resized, basename, self.output_images_dir, border=(0, 0, 0), ismask=False)
     ...
     for j in range(rh):
        for i in range(rw):
          ...
          if self.augmentation:
            ...
            self.augment(cv_image,  cropped_image_filename, output_dir, border=(0, 0, 0), ismask=ismask)
  ...
  def augment(self, image, basename, output_dir, border=(0, 0, 0), ismask=False):
    ...
    if self.resize:
      self.shrink(image, basename, output_dir, ismask)
...
if __name__ == "__main__":
  try:
    output_dir = "./Tiled-Follicular-Cell-master"
    if os.path.exists(output_dir):
      shutil.rmtree(output_dir)
    os.makedirs(output_dir)

    # Training Dataset    
    train_images_dir = "./patch dataset/training set/data/lvpao"
    train_masks_dir  = "./patch dataset/training set/label_transfered/lvpao"
    train_output_dir = os.path.join(output_dir, "train")
    os.makedirs(train_output_dir)
  
    output_images_dir = train_output_dir + "/images/"
    ...
    # Testing dataset
    test_images_dir = "./patch dataset/testing set/data/lvpao"
    test_masks_dir  = "./patch dataset/testing set/label_transfered/lvpao"
    test_output_dir = os.path.join(output_dir, "test")
    os.makedirs(test_output_dir)
    output_images_dir = test_output_dir + "/images/"
    output_masks_dir  = test_output_dir + "/masks/"
    os.makedirs(output_images_dir)
    os.makedirs(output_masks_dir)
